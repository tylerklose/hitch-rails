...
- `server_info=` accepts a callable invoked once per admitted request. It must
  ...
  String `title` and `instructions`. Raising or malformed values produce a
  generic protocol internal error without registry or SDK work. Unset, it
  defaults to the application's dasherized name and version "1.0.0".
...
reverses the install through the generator's revoke behavior; beyond that,
the host's version control is the rollback story.
...
record and returns the raw token `post_mcp` expects.
