# hitch-rails 0.3 public API

This file documents the `0.3.0` compatibility surface. It is the `0.2.0`
surface plus refresh-token issuance, rotation, and reuse detection; nothing
from 0.2 was removed or renamed. Upgrading requires running one new migration
— see [`docs/upgrading/0.2-to-0.3.md`](../upgrading/0.2-to-0.3.md).
...
`0.2.0` is published on RubyGems and is the first public release. `0.3.0` is
prepared on `main` and unreleased: the version is bumped and this document is
current, but no `v0.3.0` tag or RubyGems publication exists yet. Every tag,
GitHub release, or RubyGems publication requires an explicit maintainer
decision after its gates pass.
