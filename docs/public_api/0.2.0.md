...
the wire. M4.2 still owns the only supported successful/error result values,
output-schema checks, byte cap, and sanitized error reporting.
...
execution ordering. Closed Result values, production Redis request limiting,
and public request/invocation events remain later M4 work.
