...
# Hitch's configuration comes from the dummy app's initializer. A test calling
# Hitch.reset_configuration! otherwise leaves every later test with a blank
# configuration, reported as a failure in whichever test the seed happens to
# run next. The registry itself is prepared on first MCP use.
ActiveSupport::TestCase.teardown do
