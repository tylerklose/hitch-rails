...
`migrations`, `registry`, `hosts`, `origins`, and `rate_limit_store`. Every
check names something the host can act on; gem-self-diagnosis (package
integrity) lives in the repository's CI. The `versions` bounds come from the
loaded gemspec. JSON uses schema `hitch.doctor.v1`; every check has `status`,
stable `code`, `summary`, and bounded structural `details`.
