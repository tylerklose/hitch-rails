...
module Hitch
  class ClientAuthentication
    ...
    def self.public_client_id(client_id, body_secret_present:)
      ...
      unless valid_component?(client_id, max_bytes: Hitch::Client::MAX_CLIENT_ID_BYTES)
        raise Invalid.new("invalid_client", "Client authentication failed", http_status: :unauthorized)
      end
    ...
    def self.decode_basic(authorization)
      raise ArgumentError if authorization.bytesize > MAX_AUTHORIZATION_BYTES

      scheme, encoded = authorization.split(" ", 2)
      unless scheme&.casecmp?("Basic") && encoded.present? && !encoded.match?(/\s/)
        raise Invalid.new("invalid_client", "Client authentication failed", http_status: :unauthorized)
      end

      decoded = Base64.strict_decode64(encoded)
      encoded_client_id, encoded_secret = decoded.split(":", 2)
      raise ArgumentError if encoded_client_id.blank? || encoded_secret.nil?
